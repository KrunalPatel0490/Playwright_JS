...
class ApiMocker {
  ...
  async mockCrudOperations(resourceName, factoryMethod, initialData = []) {
    ...
    await this._mockGetAll(resourceName, data);
    await this._mockGetById(resourceName, data);
    await this._mockCreate(resourceName, data, () => nextId++);
    await this._mockUpdate(resourceName, data);
    await this._mockDelete(resourceName, data);
  }

  /**
   * Mock GET /resource - List all
   * @private
   */
  async _mockGetAll(resourceName, data) {
  ...
  /**
   * Mock GET /resource/:id - Get by ID
   * @private
   */
  async _mockGetById(resourceName, data) {
  ...
  /**
   * Mock POST /resource - Create new
   * @private
   */
  async _mockCreate(resourceName, data, getNextId) {
    await this.mockPost(
      `**/${resourceName}`,
      request => {
        const body = request.postDataJSON();
        const newItem = { ...body, id: getNextId(), createdAt: new Date().toISOString() };
        data.push(newItem);
  ...
  /**
   * Mock PUT /resource/:id - Update
   * @private
   */
  async _mockUpdate(resourceName, data) {
  ...
  /**
   * Mock DELETE /resource/:id - Delete
   * @private
   */
  async _mockDelete(resourceName, data) {
  ...
  wasApiCalled(urlPattern, method = 'GET') {
    return this.interceptedRequests.some(
      req => req.method === method && this.urlMatches(req.url, urlPattern)
    );
  }
}
