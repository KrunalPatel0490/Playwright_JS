...
class BasePage {
  ...
  /**
   * Wait for network to be idle (recommended alternative to arbitrary timeouts)
   * @param {Object} _options - Wait options (currently unused)
   * @deprecated Use more specific wait conditions instead
   */
  async waitForNetworkIdle(_options = {}) {
   ...
   * @param {Object} options - Wait options with timeout
   */
  async waitForCondition(condition, options = { timeout: 30000 }) {
    await this.page.waitForFunction(condition, null, options);
  }

  /**
   * Wait for DOM to be stable (alternative to arbitrary timeout)
  ...
  async getFrame(nameOrUrl) {
    return (
      this.page.frame({ name: nameOrUrl }) ||
